...
RSpec.describe "Relationships", type: :system do
  ...
  let!(:live_companion2) { create(:live_companion, user: user2) }
  let!(:live_companion3) { create(:live_companion, user: user3) }
  ...
  describe "フォロー中(following一覧)ページ" do
    ...
    context "ページレイアウト" do
      ...
      it "ユーザー情報が表示されていること" do
        ...
        expect(page).to have_content "ライブ同行者募集#{user.live_companions.count}件"
  ...
  describe "フィード" do
    ...
    it "フィードに自分の投稿が含まれていること" do
      expect(user.feed).to include live_companion
    end

    it "フィードにフォロー中ユーザーの投稿が含まれていること" do
      expect(user.feed).to include live_companion2
    end

    it "フィードにフォローしていないユーザーの投稿が含まれていないこと" do
      expect(user.feed).not_to include live_companion3
    end
  end
end
